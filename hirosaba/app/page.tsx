    ...
    <div className="min-h-screen pb-24">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        ...
        <div className="max-w-lg mx-auto flex border-t border-gray-100">
          {([['char', 'キャラ'], ['equip', '装備'], ['collection', 'コレクション'], ['party', 'パーティ']] as [Tab, string][]).map(([t, lbl]) => (
            <button key={t} onClick={() => setTab(t)}
              className={`flex-1 py-2.5 text-[11px] font-medium transition-colors relative ${tab === t ? 'text-slate-600' : 'text-gray-400'}`}>
              {lbl}
              {tab === t && <span className="absolute bottom-0 inset-x-3 h-0.5 bg-slate-600 rounded-full" />}
            </button>
          ))}
        </div>
      </header>
            ...
            <div className="flex gap-1 overflow-x-auto">
              ...
              {PRIORITIES.map(p => (
                <button key={p.value} onClick={() => setFilter(p.value)}
                  className={`shrink-0 text-xs px-3 py-1.5 rounded-full font-medium ${filter === p.value ? 'bg-slate-600 text-white' : 'bg-gray-100 text-gray-500'}`}>
                  {p.label}
                </button>
              ))}
            </div>
