...
function buildPrompt(b: Body): string {
  ...
  if (b.equip) cond.push(`使える器具の傾向: ${b.equip}`);
  if (b.part)  cond.push(`特に鍛えたい: ${b.part}`);

  const note = (b.equipNote ?? "").trim();
  // 設備メモがあれば、それを最優先の制約にする（無いマシンを出さないため）
  const equipBlock = note
    ? `\n\n【このジムにある設備・器具（重要）】\n${note}\n上に書かれた設備・器具でできる種目だけを提案してください。ここに無いマシン専用種目は使わないこと。判断に迷う場合は、フリーウェイト（バーベル・ダンベル）や自重でできる代替種目を選ぶ。`
    : "";

  return `あなたは親切なパーソナルトレーナーです。次の人に向けて、ジムで回す${days}分割のトレーニングメニュー（1週間のローテーション）を作ってください。
${body.length ? `体格: ${body.join(" / ")}` : "（体格の登録なし）"}
条件: ${cond.join(" / ") || "（一般的な初心者向け）"}${equipBlock}
...
- ケガ予防を最優先し、経験が浅い人には無理のない量にする。${note ? "" : "\n- 使える器具が不明な場合は、多くのジムに一般的にあるマシンやフリーウェイトに絞る。"}
