...
interface Props {
  bucket: string;                     // データの束の名前（例: kondate）
...
export default function CloudSync({ bucket, serialize, apply, onClose }: Props) {
  ...
  async function doPush() {
    ...
    try {
      await cloudPush(c, bucket, serialize());
      ...
      <div className="relative bg-white rounded-t-2xl shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-3 pb-8">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400 mt-1">在庫アプリと同じコードにすると、献立で在庫を取り込めます。</p>
          </div>
          <div className="flex gap-2">
            <button onClick={doPush} disabled={busy !== ''}
              className="flex-1 py-2.5 rounded-xl bg-orange-500 text-white text-sm font-bold disabled:opacity-50 flex items-center justify-center gap-2">
            ...
            <button onClick={doPull} disabled={busy !== ''}
              className="flex-1 py-2.5 rounded-xl border border-orange-300 text-orange-600 text-sm font-bold disabled:opacity-50 flex items-center justify-center gap-2">
              {busy === 'pull' && <span className="w-4 h-4 border-2 border-orange-200 border-t-orange-500 rounded-full animate-spin" />}
              ⬇️ クラウドから復元
            </button>
          </div>
          {msg && (
